...
void tcp_reno_init(struct sock *sk)
{
    ...
    tcp_sk(sk)->snd_cwnd = 10; // Start with a congestion window of 10
}
...
void tcp_reno_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
    ...
    if (tp->snd_cwnd <= tp->snd_ssthresh) {
        ...
        acked = tcp_slow_start(tp, acked * 2);
        if (!acked)
            return;
    } else {
        /* In "congestion avoidance", cwnd is increased by 2 full packet
